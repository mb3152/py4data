...
for Dvar in variables:
	for Ivar in variables:
		...
		#exec  ax_cmd
        axes[counter] = fig.add_subplot(ndims, ndims, counter)
        ...
        axes[counter].scatter(data[Ivar], data[Dvar], c=scolors)
		#exec ax_cmd
		print Ivar + ' vs ' + Dvar + ' ' + str(counter)
		
		if Ivar == Dvar:
			ax_cmd = 'ax' + str(counter) + '.text(.1,.9,Ivar,horizontalalignment=\'left\',transform=ax' + str(counter) + '.transAxes)'
			axes[counter].text('.1,.9,Ivar, horizontalalignment=\'left\',transform=ax' + str(counter) + '.transAxes)')
            #eval(ax_cmd)
